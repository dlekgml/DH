...
public class java_method {
	...
	public static void main(String[] args) {
		...
		mart_init(); // 제품등록이 첫번쨰 실행 되어야 한다.
		
		buyer 다희 = buyer_enroll(); // 구매자 등록 (buyer 클래스 객체 생성)
		
		put(다희); // 물건을 구매하기 위한 메서드, 더이상 구매하지 않아야 메서드가 종료된다.
	...
	static void put(buyer 다희) {
		...
		while(true) {
			...
			if(!(num>=1 && num <= mart_list.length))
				continue; // 제품번호를 잘못입력시 다시 반목문 실행;
			
			다희.basket_add(mart_list[num-1]); // 장바구니 크기 증가 - 두번째 물건을 담을때부터 
	...
	static buyer buyer_enroll() { 
		...
		buyer 이다희 = new buyer(num,name);
		이다희.basket= new mart[1]; 
	...
	static void mart_init() {
		mart_list=new mart[] {
			...
			new mart("골뱅이",2719,"통조림",1365), new mart("먹태깡",1350,"과자",4615),
